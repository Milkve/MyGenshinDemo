...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<SkillBridge.Message.Result>(translator.PushSkillBridgeMessageResult, translator.Get, translator.UpdateSkillBridgeMessageResult);
        ...
        int SkillBridgeMessageResult_TypeID = -1;
		int SkillBridgeMessageResult_EnumRef = -1;
        
        public void PushSkillBridgeMessageResult(RealStatePtr L, SkillBridge.Message.Result val)
        {
            if (SkillBridgeMessageResult_TypeID == -1)
            {
			    bool is_first;
                SkillBridgeMessageResult_TypeID = getTypeId(L, typeof(SkillBridge.Message.Result), out is_first);
				
				if (SkillBridgeMessageResult_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(SkillBridge.Message.Result));
				    SkillBridgeMessageResult_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, SkillBridgeMessageResult_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, SkillBridgeMessageResult_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for SkillBridge.Message.Result ,value="+val);
            }
			
			LuaAPI.lua_getref(L, SkillBridgeMessageResult_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out SkillBridge.Message.Result val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != SkillBridgeMessageResult_TypeID)
				{
				    throw new Exception("invalid userdata for SkillBridge.Message.Result");
				}
                ...
                {
                    throw new Exception("unpack fail for SkillBridge.Message.Result");
                }
				val = (SkillBridge.Message.Result)e;
                
            }
            else
            {
                val = (SkillBridge.Message.Result)objectCasters.GetCaster(typeof(SkillBridge.Message.Result))(L, index, null);
            }
        }
		
        public void UpdateSkillBridgeMessageResult(RealStatePtr L, int index, SkillBridge.Message.Result val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for SkillBridge.Message.Result ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(SkillBridge.Message.Result[]))
			{
			    SkillBridge.Message.Result[] array = obj as SkillBridge.Message.Result[];
				translator.PushSkillBridgeMessageResult(L, array[index]);
				return true;
			}
